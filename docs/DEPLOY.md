...
### 1. Create project and enable required APIs
...
### 5. Set GitHub repository variables and secrets
...
gh secret set FAKE_NEWS_INTEGRATION_TEXT --body "Example news text used by integration tests"
```
...
- `FAKE_NEWS_INTEGRATION_TEXT` (required by integration tests in PR workflow)
...
Integration job validates required repo variables/secrets and fails if integration
tests are skipped or if no integration tests run.
